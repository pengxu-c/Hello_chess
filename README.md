...
一个基于 EasyX 图形库的 Windows 多子棋游戏，使用 **C++17** 编写，面向对象设计，通过 CMake 构建。默认为 15×15 五子棋，支持自定义 N×N 棋盘与 n 连获胜。内置 **棋局存储管理系统**（悔棋、回访、残局保存/载入、全局统计）。
...
│   └── G20260818_160523_002.txt
└── resumes/                   残局记录（可恢复续弈）
    └── R20260818_154500_001.txt
```
